...
object ChaptersSuite extends IOSuite:
  ...
  test("Created chapters can be found") { xa =>
    ...
    for
      ...
      assetIdsSubset = NonEmptyList.fromListUnsafe(assetIds.tail)
      chaptersBefore <- chapterRepository.findByAssetId(assetIdsSubset)
      chaptersIds <- chapterRepository.create(chapters)
      chaptersAfter <- chapterRepository.findByAssetId(assetIdsSubset)
    yield expect.all(
